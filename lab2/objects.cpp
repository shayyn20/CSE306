...
Light::Light(Vector c, double r, double intensity = 200000) {
    this->center = c;
    this->radius = r;
...
Intersection Scene::closest(Ray* ray) {
    ...
    for (int i = 0; i < int(sizeof(*this->sphere_collection)/sizeof(Sphere*)); i++) {
...
Image Scene::scan(int NB_path) {
    ...
    for (int i = 0; i < H; ++i) {
        ...
        for (int j = 0; j < W; ++j) {
            ...
            int y = i;
            Vector color;
            Vector ray_direct(Qx + x + 0.25 - W/2, Qy + y + 0.25 - H/2, Qz - f);
            double offset_x;
            double offset_y;
            // printf("j = %d \n", j);
            ...
            for (int k = 0; k < NB_path; ++k) {
                boxMuller(0.5, offset_x, offset_y);
